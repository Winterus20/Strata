...
| WGSL 64-bit atomik desteği eksik (Metal) | Yüksek | Düşük | `vec2<u32>` + `atomicStoreMin` fallback |
| Node pool allocator Metal'da çalışmıyor | Orta | Orta | 32-bit `atomic<u32>` allocator kullan |
...
| **64-bit atomic node allocator** | Metal'da `atomicAdd<u64>` yok → 32-bit `atomic<u32>` allocator yeterli |
