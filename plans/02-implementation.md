...
crates/
  ...
  physics/
    ...
    ├── rapier.rs          ← bevy_rapier3d integration
  ...
  network/
    ...
    ├── renet2.rs          ← bevy_replicon_renet2 integration
    ├── replication.rs     ← bevy_replicon replication
  ...
  audio/
    ├── mod.rs             ← Audio plugin (bevy_audio)
    ├── spatial.rs         ← 3D spatial audio
    ├── ambient.rs         ← Ambient sounds
    └── registry.rs        ← Sound registry

  ui/
    ├── mod.rs             ← UI plugin (bevy_ui + glyphon)
    ├── hud.rs             ← HUD layout
...
| Terim | Açıklama |
|---|---|
| **Sector** | 32×32×32 cubic voksellik temel dünya birimi (32.768 voxel; `06-xbrickmap.md`) |
| **XBrickMap** | 3-level bitmask hiyerarşi (sector → brick 8³ → sub-brick 2³) + 4-level palet zinciri |
...
## 7. Araştırma Doğrulamaları ve Öneriler (2026-06)

> **Kaynak:** 5 worker ile 40+ WebSearch sorgusu, SIGGRAPH/akademik paper'lar, Rust/Bevy ekosistem kaynakları, karşılaştırmalı voxel motor analizi.

### 7.1 Doğrulanan Kararlar

| Karar | Doğrulama |
|-------|-----------|
| Flat `crates/` layout | matklad/rust-analyzer projesi ile academic validation |
| Domain-based crate organizasyonu | Bevy ekosistemi (100+ sub-crate) ile uyumlu |
| 6 faz uygulama planı | Incremental delivery, erken feedback döngüsü |

### 7.2 P0 — Kesin Öneriler (Phase 1)

#### `cargo-hakari` Erken Kurulumu

Bevy'nin 100+ sub-crate'i farklı feature'larla kullanılacak. `cargo-hakari` unified workspace-hack crate ile feature harmonization sağlar.

**Etki:** ~%50 build time kazancı (tekrarlanan dependency derleme eliminasyonu).
...
**Entegrasyon:** Phase 1, Hafta 1 — workspace yapısı oluşturulurken eklenmeli.

#### `lld` Linker (Windows CI + Dev)

Windows'ta MSVC linker yerine `lld` kullanımı 5-30s linking kazancı sağlar.
...
**Not:** `lld` Rust toolchain ile birlikte gelir (`rust-lld`). Ek kurulum gerekmez.

### 7.3 P1 — Önerilen Öneriler (Phase 1-2)

#### `strata-types` Crate Ayrımı

Ortak tipler (event'lar, trait tanımları, paylaşılan enum'lar) ayrı bir `strata-types` crate'ine taşınmalı. Bu:

- Client/server binary'lerinin ortak tip bağımlılığını azaltır
- Derleme paralelliğini artırır (tip değişiklikleri daha az crate'i yeniden derler)
- Circular dependency riskini minimize eder

```
crates/
  types/           ← YENİ: ortak tipler
    ├── events.rs  ← ClientBlockBreakRequest, ServerBlockBrokenBroadcast vb.
    ├── traits.rs  ← Mesher, FluidSolver vb. trait'ler
    └── enums.rs   ← Tier, MeshType, WeatherType vb.
  core/            ← types'a bağımlı
  ecs/             ← types'a bağımlı
  ...
```

**Kriter:** 3+ crate'in kullandığı tip → `strata-types`'a taşınır. Phase 2'de crate sayısı artarken değerlendir.
