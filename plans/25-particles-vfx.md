...
```rust
/// Particle system — GPU compute tabanlı.
...
fn simulate_particles(
    ...
    // Yerçekimi
    particle.velocity.y -= particle.gravity * params.delta_time;

    // Drag
    let drag_factor = 1.0 - particle.drag * params.delta_time;
    ...
    if (particle.collision) {
        let grid_pos = floor(vec3f(particle.position));
...
```wgsl
// Particle render shader (point sprite)
...
@vertex
fn particle_vs(input: ParticleInput) -> VertexOutput {
    var output: VertexOutput;

    // Screen-space size
    let view_pos = view_matrix * vec4f(input.position, 1.0);
    let screen_pos = projection_matrix * view_pos;

    output.position = screen_pos;
    output.color = input.color;
    output.size = input.size / screen_pos.w; // Perspective size

    return output;
}

@fragment
fn particle_fs(input: FragmentInput) -> @location(0) vec4f {
    // Circular particle
    ...
    // Soft edge
    let alpha = smoothstep(0.5, 0.3, dist) * input.color.a;
...
crates/
  particles/
    ...
    ├── compute/
    │   ├── mod.rs          ← Compute shader
    │   └── simulation.wgsl ← Simulation compute shader
    ├── render/
    │   ├── mod.rs          ← Particle render
    │   ├── pipeline.rs     ← Render pipeline
    │   └── shader.wgsl     ← Particle shader
