...
use crate::rng::{Pcg32, WORLD_SEED, wyhash};
...
/// Decide the block at a world coordinate (AIR sentinel = 0 for empty).
fn select_block(x: i32, y: i32, z: i32, wb: &WorldBlocks) -> BlockId {
    let air = BlockId::AIR;
    ...
    let d = density(x as f32, y as f32, z as f32, biome);
    if d <= 0.0 {
...
fn col_rng(x: i32, z: i32) -> Pcg32 {
    ...
    Pcg32::new(wyhash(mixed, WORLD_SEED ^ TREE_SALT), 0xA3C1)
}
    ...
    for lx in 0..32u32 {
        for ly in 0..32u32 {
            for lz in 0..32u32 {
                ...
                let wy = oy + ly as i32;
                let wz = oz + lz as i32;
                let b = select_block(wx, wy, wz, &wb);
                if b != BlockId::AIR {
